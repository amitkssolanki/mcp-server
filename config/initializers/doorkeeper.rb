...
Doorkeeper.configure do
  ...
  #
  # The resource owner here is the Spree *admin*, not a storefront customer:
  # every MCP tool this server exposes (revenue, orders, customer PII, and
  # the write tools that change prices/order status) is store-operator data.
  # Whoever authorizes an OAuth client is granting it admin-equivalent access,
  # so they must go through the same login as the admin panel.
  resource_owner_authenticator do
    current_legacy_admin_user || begin
      store_location_for(:legacy_admin_user, request.original_url)
  ...
  #
  # Devise's own helpers (current_legacy_admin_user, store_location_for) are
  # hooked into every ActionController::Base subclass automatically, but
  # spree_admin_login_path (Spree::AuthenticationHelpers) is only included
  # into our own ApplicationController — resource_owner_authenticator above
  # needs it, so Doorkeeper's controllers must inherit from ours.
  ...
  #
  # DCR-registered clients (Claude included) are non-confidential — they get
  # no client_secret, so PKCE is their only protection against authorization
  # code interception. OAuth 2.1 makes this mandatory.
  force_pkce
  ...
  #
  # mcp:read is granted to every token; mcp:write must be explicitly
  # requested by the client and approved on the consent screen. McpController
  # derives which tool set to expose (9 read tools, or all 11) from which of
  ...
  #
  # Only authorization_code — not the default (authorization_code +
  # client_credentials). client_credentials mints a token straight from a
  # client_id/secret with no resource-owner step at all, which would let a
  # registered client skip the admin consent screen entirely. Every token
  # here should trace back to an admin explicitly clicking "Allow."
